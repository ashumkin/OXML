...
{$IFDEF CONDITIONALEXPRESSIONS}
  ...
  {$IFEND}
  {$IF CompilerVersion >= 22.0}  // Delphi XE or newer
    {$DEFINE OmniXmlUtils_DXE}
  {$IFEND}
  {$IF CompilerVersion >= 23.0}  // Delphi XE2 or newer
    {$DEFINE OmniXmlUtils_DXE2}
  {$IFEND}
{$ENDIF}

{$IF DEFINED(OmniXmlUtils_DXE) OR DEFINED(FPC)}
  {$DEFINE OmniXmlUtils_FormatSettings}
{$IFEND}
...
{$IFDEF MSWINDOWS}
  ...
  function XMLSaveToRegistry(xmlDocument: IXMLDocument; rootKey: HKEY;
    const key, value: string; {%H-}outputFormat: TOutputFormat): boolean;
{$ENDIF}
  ...
  try
    ...
    try
      repeat
        ...
        if Base64Decode(EncBuffer, DecBuffer, EncSize, {%H-}DecSize) then
...
begin
  Result := StrToFloat(StringReplace(nodeValue, DEFAULT_DECIMALSEPARATOR,
    {$IFDEF OmniXmlUtils_FormatSettings}FormatSettings.{$ENDIF}DecimalSeparator, [rfReplaceAll]));
end; { XMLStrToReal }

function XMLStrToRealDef(nodeValue: XmlString; defaultValue: real): real;
begin
  if not XMLStrToReal(nodeValue,{%H-}Result) then
    Result := defaultValue;
end; { XMLStrToRealDef }
...
function XMLStrToExtendedDef(nodeValue: XmlString; defaultValue: extended): extended;
begin
  if not XMLStrToExtended(nodeValue, {%H-}Result) then
    Result := defaultValue;
...
function XMLStrToCurrencyDef(nodeValue: XmlString; defaultValue: Currency): Currency;
begin
  if not XMLStrToCurrency(nodeValue, {%H-}Result) then
    Result := defaultValue;
...
function XMLStrToIntDef(nodeValue: XmlString; defaultValue: integer): integer;
begin
  if not XMLStrToInt(nodeValue,{%H-}Result) then
    Result := defaultValue;
end; { XMLStrToIntDef }
...
function XMLStrToInt64Def(nodeValue: XmlString; defaultValue: int64): int64;
begin
  if not XMLStrToInt64(nodeValue,{%H-}Result) then
    Result := defaultValue;
...
function XMLStrToBool(nodeValue: XmlString): boolean; overload;
begin
  if not XMLStrToBool(nodeValue, {%H-}Result) then
...
begin
  if not XMLStrToDateTime(nodeValue, {%H-}Result) then
...
begin
  if not XMLStrToDate(nodeValue, {%H-}Result) then
...
begin
  if not XMLStrToTime(nodeValue, {%H-}Result) then
...
function GetNodeTextStr(parentNode: IXMLNode; nodeTag: string;
  defaultValue: XmlString): XmlString;
begin
  if not GetNodeText(parentNode, nodeTag, {%H-}Result) then
    Result := defaultValue
  else
    Result := Trim(Result);
end; { GetNodeTextStr }
...
var
  nodeText: XmlString;
begin
  if not GetNodeText(parentNode, nodeTag, {%H-}nodeText) then
    Result := defaultValue
  else
...
function GetNodeAttrStr(parentNode: IXMLNode; attrName: string;
  defaultValue: XmlString): XmlString;
begin
  if not GetNodeAttr(parentNode, attrName, {%H-}Result) then
    Result := defaultValue
  else
    Result := Trim(Result);
end; { GetNodeAttrStr }
...
var
  attrValue: XmlString;
begin
  if not GetNodeAttr(parentNode,attrName,{%H-}attrValue) then
    Result := defaultValue
  else
...
begin
  Result := StringReplace(FloatToStr(value),
    {$IFDEF OmniXmlUtils_FormatSettings}FormatSettings.{$ENDIF}DecimalSeparator, DEFAULT_DECIMALSEPARATOR, [rfReplaceAll]);
end; { XMLRealToStr }
  ...
  while iNode < parentNode.ChildNodes.Length do begin
    ...
    if childNode.NodeType = ELEMENT_NODE then begin
      ...
      if matches and (matchesName <> '') then
        matches := (SameText(childNode.NodeName, matchesName));
      if matches then
        matches := GetNodeAttr(childNode, attributeName, {%H-}attrValue);
